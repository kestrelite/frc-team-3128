...
public class Beaglebone extends Event
{
    ...
    /**
     * read bytes from serial port into commandInProgress
     * stops if it finds an SOSProtocol.END_TRANSMISSION
     * @return 
     */
    void readCommandBytes()
    {
            try 
            {
                if(inStream.available() > 0)
                {
                    ...
                    if(data == -1)
                    {
                        DebugLog.log(DebugLog.LVL_INFO, this, "Got EOF.");
                        return;
                    }
                    commandInProgress[placeInCommandInProgress + 1] = (byte)data;
                    //DebugLog.log(DebugLog.LVL_INFO, this, "Beaglebone read byte" + Integer.toHexString(data) + " into slot " + (placeInCommandInProgress + 1));
    ...
    public void execute()
    {
        ...
        if((placeInCommandInProgress > -1) && (commandInProgress[placeInCommandInProgress] == SOSProtocol.END_TRANSMISSION))
        {
            ...
            //DebugLog.log(DebugLog.LVL_INFO, this, "Beaglebone read " + command.toString());
            
            placeInCommandInProgress = 0;
            
            DebugLog.log(DebugLog.LVL_INFO, this, "Done parsing.");
        }
    }
