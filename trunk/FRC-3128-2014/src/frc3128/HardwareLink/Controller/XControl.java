...
public class XControl extends Event {
    ...
    public String getButtonKey(String button, boolean pressed) {
        return "button" + this.port + "-" + button + (pressed ? "Down" : "Up");
    }
    /**
    ...
    public void execute() {
        ...
        if (x1 != xControl.getRawAxis(XControlMap.X1_AXIS))
            updateJoy1 = true;
        if (x2 != xControl.getRawAxis(XControlMap.X2_AXIS))
            updateJoy2 = true;
        if (y1 != xControl.getRawAxis(XControlMap.Y1_AXIS))
            updateJoy1 = true;
        if (y2 != xControl.getRawAxis(XControlMap.Y2_AXIS))
            updateJoy2 = true;
        if (triggers != xControl.getRawAxis(XControlMap.TRIGGER_AXIS))
            updateTriggers = true;
